...
function generatePDF() {
    ...
    const feedback = (analysisResults.improvementSuggestions || []).map(
        (item) => `${item.category}: ${item.text || "No suggestion provided"}`
    );
    ...
    doc.text(`${fitScore}%`, textX, centerY + 4);
    ...
    matchedKeywords.forEach((keyword, index) => {
        doc.text(`- ${keyword}`, 10, yPosition + 10 + index * 10);
    });

    // Feedback Section 
    yPosition += 20 + matchedKeywords.length * 10;
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Feedback:", 10, yPosition);

    // make sure all text is inside page
    feedback.forEach((item, index) => {
        const wrappedText = doc.splitTextToSize(item, 190); 
        doc.setFont("helvetica", "normal");
        doc.text(wrappedText, 10, yPosition + 10 + index * 10);
